...
def discover(pattern: str = "*") -> list[CaseSpec]:
    ...
    if pattern.startswith("tag:"):
        tag = pattern[4:]
        return sorted(
            [spec for spec in _REGISTRY.values() if tag in spec.tags], key=lambda spec: spec.name
        )
    return sorted(
        [spec for spec in _REGISTRY.values() if fnmatch(spec.name, pattern)],
...
def _import_case_modules() -> None:
    ...
    for path in sorted(cases_dir.glob("*.py")):
        if path.name.startswith("_"):
            continue
        _import_module(f"tests.benchmarks.cases.{path.stem}")
