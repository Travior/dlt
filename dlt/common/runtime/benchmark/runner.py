...
from dlt.common.runtime.benchmark.model import Event, EventSet, RunResult, Stat
from dlt.common.runtime.benchmark.registry import CaseSpec, get_case
...
def run(case: str | CaseSpec, runs: Optional[int] = None, save: Optional[str] = None) -> RunResult:
    ...
    _log(f"case={spec.name} runs={repeat_count} warmup={spec.warmup} status=start")
    events: list[Event] = []
    ...
    for run_index in range(-spec.warmup, repeat_count):
        phase = "warmup" if run_index < 0 else "run"
        run_label = run_index if run_index < 0 else f"{run_index + 1}/{repeat_count}"
        _log(f"case={spec.name} phase={phase} run={run_label} status=start")
        ...
        if run_index < 0:
            _log(
                f"case={spec.name} phase={phase} run={run_label} "
                f"wall={float(sidecar['wall']):.3f}s status=done"
            )
            continue
        ...
        _log(
            f"case={spec.name} phase={phase} run={run_label} "
            f"wall={wall_samples[-1]:.3f}s cpu={cpu_samples[-1]:.3f}s "
            f"peak_rss={int(rss_samples[-1])} status=done"
        )
    ...
    if save:
        path = result.save(save)
        _log(f"case={spec.name} save={save} path={path} status=saved")
    _log(f"case={spec.name} status=done")
    return result
...
def _run_worker(spec: CaseSpec, run_index: int, result_dir: Path, tmp_dir: Path) -> dict[str, Any]:
    ...
    cmd = [
        sys.executable,
        "-m",
        "dlt.common.runtime.benchmark.runner",
        "--worker",
    ...
    sidecar = json.loads(result_dir.joinpath("_worker.json").read_text(encoding="utf-8"))
